                ...
                return (
                  <div className="rounded-2xl border border-slate-200 dark:border-white/10 bg-white dark:bg-[#0a0a0a] overflow-hidden">
                    <div className="px-5 py-4 border-b border-slate-100 dark:border-white/5 flex justify-between items-center">
                      ...
                      <span className="text-xs font-semibold text-teal-600 dark:text-teal-400 font-mono">Total: {o.totalQty} pcs</span>
                    </div>
                    <table className="w-full text-sm">
                      ...
                      <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                        {items.map((it: any, idx: number) => (
                          <tr key={idx}>
                            ...
                            <td className="py-2.5 pl-3 pr-5 text-right tabular-nums font-bold text-teal-600 dark:text-teal-400">{fmtCurrency(Number(it.qty || 0) * o.unitPrice)}</td>
                          </tr>
                        ))}
                      </tbody>
                      <tfoot className="bg-slate-50 dark:bg-white/5 font-bold text-sm">
                        <tr>
                          ...
                          <td className="py-3 px-3 text-right tabular-nums text-teal-600 dark:text-teal-400">{o.totalQty}</td>
                          <td />
                          <td className="py-3 pl-3 pr-5 text-right tabular-nums text-teal-600 dark:text-teal-400">{fmtCurrency(o.totalAmount)}</td>
                        </tr>
                      </tfoot>
                    </table>
                  </div>
                      ...
                      {sizeKeys.map((size) => {
                        ...
                        return (
                          <div key={size} className="rounded-xl border border-slate-100 dark:border-white/5 p-4 bg-slate-50/50 dark:bg-white/5 text-center">
                            ...
                            <span className="block text-xs font-semibold text-teal-600 dark:text-teal-400 mt-2">{fmtCurrency(qty * o.unitPrice)}</span>
                          </div>
                        )
                      })}
                    </div>
