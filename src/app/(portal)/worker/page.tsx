...
import { serializeOrder, fmtDate, getServiceLabel, getStatusLabel } from '@/lib/utils'
    ...
    <div className="space-y-6 max-w-5xl">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[
          ...
          { label: 'Completed',      value: sm['COMPLETED']?? 0, icon: CheckCircle2,color: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400' },
                    ...
                    <tr key={o.id} className="hover:bg-slate-50/50 dark:hover:bg-white/5 transition-colors">
                      ...
                      <td className="py-3 pl-3 pr-5">
                        <Link href={`/worker/orders/${o.id}`}
                          className="rounded-lg bg-slate-100 dark:bg-white/10 px-3 py-1.5 text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-teal-50 hover:text-teal-700 dark:hover:bg-teal-500/20 dark:hover:text-teal-300 transition-colors">
                          Update →
                        </Link>
                      </td>
                    </tr>
